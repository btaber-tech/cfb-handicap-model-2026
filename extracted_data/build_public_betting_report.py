...
all but one game behind a free account login) and cross-references each
game's % of bets / % of money against our own polled spread history in
line_movement_2026_wkN.csv (see build_line_movement_report.py) to flag
real Reverse Line Movement: the spread actually moving AGAINST the
majority of tickets (bets%), which is the standard definition -- not
just a bets/money split sitting there statically.

RLM logic: for each game with nonzero real spread movement (from our own
polled Bovada/DraftKings snapshots), check whether the line moved toward
the side with the MINORITY of bets%. If so, flag it -- money% is shown
alongside as the likely explanation (bigger/fewer bets outweighing more
...
def build_game_card(g, movement_by_pair):
    ...
    real_move = movement_by_pair.get(key)  # list of dicts with provider/spread_move
    ...
    if real_move:
        for m in real_move:
            ...
            if moved_toward != majority_side:
                rlm = {"provider": m["provider"], "move": move, "moved_toward": moved_toward}
                break
    ...
    if rlm:
        ...
        rlm_html = (f'<div class="rlm-flag">&#9888; Reverse line movement: {esc(rlm["provider"])}\'s '
                    f'spread moved {rlm["move"]:+.1f} pts toward <strong>{esc(towards_team)}</strong>, '
                    f'the side with the <em>minority</em> of bets &mdash; against the crowd, with the money.</div>')
...
<div class="wrap">
  <header class="masthead">
    ...
    data posted so far, cross-checked against our own polled spread history to flag actual reverse line
  ...
  <div class="callout">
    ...
    RLM</strong> by comparing the majority ticket-count side (bets%) against real spread movement pulled
    from our own 6-hourly line snapshots (not VegasInsider's own open/consensus figures, which blend
    multiple books and aren't a clean single-book time series). A flag only fires when the line actually
    moved against the crowd &mdash; most games here haven't moved at all yet, so most show no flag simply
    because there's nothing to be reverse yet.
  </div>
